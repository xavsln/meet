...
const credentials = {
  ...
  redirect_uris: ["https://xavsln.github.io/meet/"],
  javascript_origins: ["https://xavsln.github.io", "http://localhost:3000"],
};
...
// The second step is where you need to use the code from the first step to generate an access token.
...
module.exports.getCalendarEvents = (event) => {
  ...
  oAuth2Client.setCredendials({ access_token });
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((results) => {
      // Respond with event results
      console.log("Results From the getCalendar", results);
      return {
        statusCode: 200,
        headers: {
          "Access-Control-Allow-Origin": "*",
